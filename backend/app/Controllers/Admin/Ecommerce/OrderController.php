...
final class OrderController
{
    ...
    public function status(int $id): void
    {
        ...
        $db = Database::connection();
        $orderStmt = $db->prepare('select * from orders where id = :id and module_key = :module_key limit 1');
        $orderStmt->execute(['id' => $id, 'module_key' => $this->moduleKey]);
        $order = $orderStmt->fetch();
        $stmt = $db->prepare('update orders set order_status = :status, updated_at = CURRENT_TIMESTAMP where id = :id and module_key = :module_key');
    ...
    public function assignDelivery(int $id): void
    {
        ...
        if ($deliveryManId > 0) {
            ...
            $stmt = $db->prepare('update orders set delivery_man_id = :delivery_man_id, delivery_assigned_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP where id = :id and module_key = :module_key');
            $stmt->execute(['delivery_man_id' => $deliveryManId, 'id' => $id, 'module_key' => $this->moduleKey]);
            OrderStatusHistory::record($id, null, 'delivery_assigned', 'admin', $_SESSION['admin_name'] ?? 'Admin', 'Assigned to ' . $deliveryMan['name']);
            NotificationLog::record('delivery', $deliveryManId, null, 'Delivery assigned', 'Order ' . ($order['order_number'] ?? ('#' . $id)) . ' was assigned to you.', $id, $this->moduleKey);
            if ($order) {
            ...
            $stmt = $db->prepare('update orders set delivery_man_id = null, delivery_assigned_at = null, updated_at = CURRENT_TIMESTAMP where id = :id and module_key = :module_key');
            $stmt->execute(['id' => $id, 'module_key' => $this->moduleKey]);
    ...
    public function tracking(int $id): void
    {
        ...
        $stmt = Database::connection()->prepare(
            'update orders set tracking_provider = :tracking_provider, tracking_number = :tracking_number, tracking_url = :tracking_url, expected_delivery = :expected_delivery, updated_at = CURRENT_TIMESTAMP where id = :id and module_key = :module_key'
        );
        $stmt->execute([
            'id' => $id,
