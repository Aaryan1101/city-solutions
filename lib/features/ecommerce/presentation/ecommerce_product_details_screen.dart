...
class EcommerceProductDetailsScreen extends StatefulWidget {
  const EcommerceProductDetailsScreen({
    super.key,
  ...
  @override
  State<EcommerceProductDetailsScreen> createState() =>
      _EcommerceProductDetailsScreenState();
}

class _EcommerceProductDetailsScreenState
    extends State<EcommerceProductDetailsScreen> {
  int _quantity = 1;
  int _imageIndex = 0;
  EcommerceProductVariant? _variant;
  bool _adding = false;
  bool _wishlistLoading = false;
  bool _wishlisted = false;
  late Future<EcommerceProductReviewsData> _reviewsFuture;
  ...
  Widget build(BuildContext context) {
    ...
    final isMedical = widget.apiBaseUrl?.contains('/medical') == true;
    final isBlockedMedical =
        isMedical && product.medicineType == 'blocked_online';
    final accent =
        isMedical ? const Color(0xFFE94C3D) : const Color(0xFF2F6BF0);
    final heroColors = isMedical
        ? const [Color(0xFFFFE6EF), Color(0xFFFFD6DE)]
        : const [Color(0xFFE3ECFF), Color(0xFFCFE0FF)];
    final savedAmount =
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: FilledButton.icon(
                    onPressed: _adding || stock <= 0 || isBlockedMedical
                        ? null
                        : _addToCart,
                    icon: _adding
                        ? const AppSkeletonBox(width: 18, height: 18, radius: 9)
                        : const Icon(Icons.add_shopping_cart_rounded),
                    label: Text(isBlockedMedical
                        ? 'Contact pharmacy'
                        : _adding
                            ? 'Adding...'
                            : 'Add to cart'),
                    ...
                    children: [
                      ...
                      if (isMedical &&
                          (product.requiresPrescription ||
                              product.requiresAgeConfirmation ||
                              product.medicineType == 'blocked_online')) ...[
                        const SizedBox(height: 14),
                        _MedicalSafetyCard(product: product, accent: accent),
                      ],
                      const SizedBox(height: 14),
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: _TrustCard(
                              ...
                              title:
                                  isMedical ? 'Safe Delivery' : 'Fast Delivery',
                            ),
                          ),
                        ],
                      ),
...
class _MedicalSafetyCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final title = switch (product.medicineType) {
      'blocked_online' => 'Not available for online ordering',
      'restricted' => 'Pharmacist review required',
      'prescription_required' => 'Prescription required',
      _ when product.requiresPharmacistReview => 'Pharmacist review required',
      _ => 'Medical safety confirmation',
    };
    final details = <String>[
      if (product.scheduleTag.isNotEmpty) product.scheduleTag,
      if (product.maxQtyPerOrder != null)
        'Max ${product.maxQtyPerOrder} per order',
      if (product.maxQtyPerMonth != null)
        'Max ${product.maxQtyPerMonth} per month',
      if (product.requiresAgeConfirmation) 'Age confirmation required',
      if (product.requiresPrescription)
        'Upload prescription details during checkout',
    ];
...
class _ReviewsSection extends StatelessWidget {
  ...
  final Future<EcommerceProductReviewsData> future;
  final VoidCallback onWriteReview;

  @override
  Widget build(BuildContext context) {
    return FutureBuilder<EcommerceProductReviewsData>(
      future: future,
...
class _ReviewTile extends StatelessWidget {
  ...
  final EcommerceProductReview review;
...
class _ReviewSheet extends StatefulWidget {
  const _ReviewSheet({required this.productId, required this.apiBaseUrl});
...
class _ReviewSheetState extends State<_ReviewSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      final session = await EcommerceSessionStore().load();
      await EcommerceApiClient(baseUrl: widget.apiBaseUrl).saveProductReview(
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      child: Ink(
        ...
        child: Row(
          children: [
            const Icon(Icons.storefront_outlined, color: Color(0xFF6C5CE7)),
            const SizedBox(width: 10),
            Expanded(
              child: Text(
