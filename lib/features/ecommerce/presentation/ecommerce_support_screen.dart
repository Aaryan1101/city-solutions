...
class EcommerceSupportScreen extends StatefulWidget {
  const EcommerceSupportScreen({
    super.key,
  ...
  @override
  State<EcommerceSupportScreen> createState() => _EcommerceSupportScreenState();
}

class _EcommerceSupportScreenState extends State<EcommerceSupportScreen> {
  late final EcommerceApiClient _api;
  final _subjectController = TextEditingController();
  final _messageController = TextEditingController();
  final _attachmentController = TextEditingController();
  late Future<List<EcommerceSupportThread>> _future;
  bool _sending = false;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          FutureBuilder<List<EcommerceSupportThread>>(
            future: _future,
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return const AppSkeletonList(cardCount: 3);
              }
              final threads = snapshot.data ?? const <EcommerceSupportThread>[];
              if (threads.isEmpty) {
                          ...
                          onTap: () async {
                            await Navigator.of(context).push(
                              MaterialPageRoute<void>(
                                builder: (_) => EcommerceSupportThreadScreen(
...
class EcommerceSupportThreadScreen extends StatefulWidget {
  const EcommerceSupportThreadScreen({
    super.key,
    required this.session,
  ...
  @override
  State<EcommerceSupportThreadScreen> createState() =>
      _EcommerceSupportThreadScreenState();
}

class _EcommerceSupportThreadScreenState
    extends State<EcommerceSupportThreadScreen> {
  late final EcommerceApiClient _api;
  final _messageController = TextEditingController();
  final _attachmentController = TextEditingController();
  Timer? _pollTimer;
  late Future<EcommerceSupportThreadDetails> _future;
  bool _sending = false;
  int _pollSeconds = 15;
  ...
  Future<EcommerceSupportThreadDetails> _load() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<EcommerceSupportThreadDetails>(
        future: _future,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting &&
              !snapshot.hasData) {
          ...
          final messages =
              details?.messages ?? const <EcommerceSupportMessage>[];
...
class _MessageBubble extends StatelessWidget {
  ...
  final EcommerceSupportMessage message;
