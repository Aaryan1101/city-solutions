...
class MartAddressScreen extends StatefulWidget {
  const MartAddressScreen({
    super.key,
  ...
  @override
  State<MartAddressScreen> createState() => _MartAddressScreenState();
}

class _MartAddressScreenState extends State<MartAddressScreen> {
  final _api = MartApiClient();
  late Future<List<MartAddress>> _future;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<MartAddress>>(
        future: _future,
        builder: (context, snapshot) {
          ...
          final addresses = snapshot.data ?? const <MartAddress>[];
  ...
  Future<void> _addAddress() async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => MartAddressFormScreen(session: widget.session),
      ),
    );
  ...
  Future<void> _setDefault(MartAddress address) async {
  ...
  Future<void> _delete(MartAddress address) async {
...
class MartAddressFormScreen extends StatefulWidget {
  const MartAddressFormScreen({super.key, required this.session});

  final MartCustomerSession session;

  @override
  State<MartAddressFormScreen> createState() => _MartAddressFormScreenState();
}

class _MartAddressFormScreenState extends State<MartAddressFormScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      await MartApiClient().saveAddress(
        token: widget.session.token,
