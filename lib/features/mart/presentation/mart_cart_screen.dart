...
import 'mart_checkout_screen.dart';
import 'mart_remote_image.dart';

class MartCartScreen extends StatefulWidget {
  const MartCartScreen({super.key, required this.guestId, this.apiBaseUrl});
...
class _MartCartScreenState extends State<MartCartScreen> {
  ...
  late Future<MartCartData> _future;
  bool _busy = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<MartCartData>(
        future: _future,
        builder: (context, snapshot) {
          ...
          if (cart.items.isEmpty) {
            return const _CartMessage(
              ...
              message: 'Add products from Mart to continue checkout.',
            );
          }
                ...
                onCheckout: _busy || cart.minimumOrderRemaining > 0
                    ...
                    : () async {
                        final placed = await Navigator.of(context).push<bool>(
                          MaterialPageRoute(
                            builder: (_) => MartCheckoutScreen(
                              guestId: widget.guestId,
                              cart: cart,
                              sessionFuture: MartSessionStore().load(),
  ...
  Future<void> _update(MartCartItem item, int quantity) async {
    setState(() => _busy = true);
    try {
  ...
  Future<void> _remove(MartCartItem item) async {
    setState(() => _busy = true);
    try {
              ...
              children: [
                ...
                Text('₹${item.total.toStringAsFixed(0)}',
                    style: const TextStyle(
                        color: Color(0xFF20A66A), fontWeight: FontWeight.w900)),
              ],
            ),
          ),
          Column(
                ...
                children: [
                  ...
                  if (cart.couponDiscount > 0) ...[
                    ...
                    _CartTotalLine(
                      ...
                      valueColor: const Color(0xFF20A66A),
                    ),
                  ],
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                child: Icon(icon, color: const Color(0xFF20A66A), size: 40),
              ),
