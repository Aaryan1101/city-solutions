...
class MartOrderHistoryScreen extends StatefulWidget {
  const MartOrderHistoryScreen({
    super.key,
    required this.guestId,
    this.showAppBar = true,
    this.apiBaseUrl,
    this.title = 'My Orders',
    this.emptyTitle = 'No mart orders yet.',
...
class _MartOrderHistoryScreenState extends State<MartOrderHistoryScreen> {
  ...
  late Future<List<MartOrder>> _future;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: FutureBuilder<List<MartOrder>>(
          future: _future,
          builder: (context, snapshot) {
            ...
            final orders = snapshot.data ?? const <MartOrder>[];
            if (orders.isEmpty) {
              return _OrderMessage(
...
class MartOrderDetailsScreen extends StatefulWidget {
  const MartOrderDetailsScreen({
    super.key,
  ...
  @override
  State<MartOrderDetailsScreen> createState() => _MartOrderDetailsScreenState();
}

class _MartOrderDetailsScreenState extends State<MartOrderDetailsScreen> {
  late final MartApiClient _api;
  late Future<MartOrderDetails> _future;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<MartOrderDetails>(
        future: _future,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const AppSkeletonPage();
  ...
  Future<void> _cancelOrder(MartOrder order) async {
  ...
  Future<void> _reportOrderIssue(MartOrder order) async {
    final session = await MartSessionStore().load();
    if (!mounted) return;
    await Navigator.of(context).push(
      MaterialPageRoute<void>(
        builder: (_) => ComplaintScreen(
          initialModuleKey: 'mart',
  ...
  Future<void> _requestRefund(MartOrder order, MartOrderItem item) async {
...
class MartRefundsScreen extends StatefulWidget {
  const MartRefundsScreen({
    super.key,
    required this.guestId,
    this.apiBaseUrl,
    this.showAppBar = true,
  ...
  @override
  State<MartRefundsScreen> createState() => _MartRefundsScreenState();
}

class _MartRefundsScreenState extends State<MartRefundsScreen> {
  late final MartApiClient _api;
  late Future<List<MartRefundRequest>> _future;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Theme.of(context).scaffoldBackgroundColor,
      appBar: widget.showAppBar ? AppBar(title: Text(widget.title)) : null,
      body: FutureBuilder<List<MartRefundRequest>>(
        future: _future,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const AppSkeletonPage();
          ...
          final refunds = snapshot.data ?? const <MartRefundRequest>[];
          if (refunds.isEmpty) {
            return _OrderMessage(
              icon: Icons.assignment_return_outlined,
              title: widget.emptyTitle,
...
class _OrderItemRow extends StatelessWidget {
  ...
  final MartOrderItem item;
  final MartRefundRequest? refund;
...
class _StatusPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color = switch (effectiveTone) {
      ...
      _StatusTone.success => const Color(0xFF20A66A),
